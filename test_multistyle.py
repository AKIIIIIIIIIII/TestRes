...
with torch.no_grad():
    styleCount = 0
    for styleItem in os.listdir(opts.style):
        multstyle=os.path.join(opts.style,styleItem)
        ...
        for item in os.listdir(opts.input):
            ...
            count += 1
            image = Variable(transform(Image.open(multInput).convert('RGB')).unsqueeze(0).cuda())
